...
class AIPlayer:
    ...
    def nega_alpha_beta(self, board: Board, depth: int, pm: list, alpha: int, beta: int, pion: PION):
        ...
        if depth != 0:
            moves = board.possible_moves(pion)
        else:
            moves = pm
        ...
        for move in moves:
            ...
            board_copy.update_state(move, pion) 
    ...
    def negamax(self, board: Board, depth: int, pm: list, pion: PION):
        """
        Joue un coup en utilisant l'algorithme negamax
        ...
        for move in moves:
            board_copy = board.copy()
            board_copy.update_state(move, pion) 
            score = -self.negamax(board_copy, depth+1, None, pion.other_type())[0]
